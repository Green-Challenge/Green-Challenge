...
public class ChallengeService {
    ...
    public List<ChallengeChartResponseDTO> getChallengeChart(ChallengeChartRequestDTO challengeChartRequestDTO){
        ...
        List<MovementLog> nowMonth = movementLogRepository.findByDayGreaterThanAndDayLessThanEqualAndAndTransportationAndUser(start, end, trans, user)
                .stream().map(Optional::orElseThrow).collect(Collectors.toList());
        List<MovementLog> lastMonth = movementLogRepository.findByDayGreaterThanAndDayLessThanEqualAndAndTransportationAndUser(start.minusMonths(1), start, trans, user)
                .stream().map(Optional::orElseThrow).collect(Collectors.toList());
